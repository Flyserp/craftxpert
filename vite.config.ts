  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          // Branding assets (favicon / uploaded logos) — always revalidate so
          // an admin-uploaded icon appears without a hard refresh.
          {
            urlPattern: ({ url }) =>
              /\/(favicon|apple-touch-icon)/i.test(url.pathname) ||
              /\/storage\/v1\/object\/public\/branding\//.test(url.pathname),
