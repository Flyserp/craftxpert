...
CREATE OR REPLACE FUNCTION public.bulk_moderation_action(
  _items jsonb,           -- [{"kind":"verification","entity_id":"..."}, ...]
  _action text,           -- 'approve' | 'reject' | 'dismiss' | 'request_info'
...
BEGIN
  ...
  END IF;
  IF _action NOT IN ('approve','reject','dismiss','request_info') THEN
      ...
      ELSIF v_kind = 'dispute' THEN
        UPDATE public.disputes
           SET status = CASE _action
                          ...
                          WHEN 'request_info' THEN 'info_requested'
                        END,
               resolved_by = CASE WHEN _action IN ('approve','reject','dismiss') THEN v_uid ELSE resolved_by END,
               resolved_at = CASE WHEN _action IN ('approve','reject','dismiss') THEN now() ELSE resolved_at END,
               admin_notes = COALESCE(_note, admin_notes), updated_at = now()
         WHERE id = v_id;
