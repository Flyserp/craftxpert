...
BEGIN
  ...
  -- Auto-reject if wallet is short. Persist the change and return a soft-failure JSON
  -- (returning instead of raising so the UPDATE/notification/audit aren't rolled back).
